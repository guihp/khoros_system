...
interface ProfissionalReviewsProps {
  avaliacao: AvaliacaoSummary;
}

export function ProfissionalReviews({ avaliacao }: ProfissionalReviewsProps) {
  const { itens, media, total } = avaliacao;
  ...
  return (
    <section className="rounded-card border border-calm-200 bg-white/90 p-5 sm:p-6">
      ...
      <p className="mt-1 text-sm text-calm-600">
        Feedback publicado por pacientes após consultas nesta plataforma.
      </p>

      {itens.length === 0 ? (
        <div className="mt-5 rounded-lg border border-dashed border-calm-200 bg-calm-50/60 px-4 py-8 text-center">
          <p className="text-sm font-medium text-calm-800">Ainda sem avaliações públicas</p>
          <p className="mt-1 text-sm text-calm-600">
            As primeiras notas e comentários aparecerão aqui após as consultas.
          </p>
        </div>
      ) : (
